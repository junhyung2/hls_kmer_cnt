...
/* exp 40: 2-XOR hash + II=1 (vs exp 39: 4-XOR hash + II=1)
 *
 * Problem in exp 39:
 *   II=1 pragma → EXTRACT_LOOP critical path with 4-XOR hash (4 XOR+shift ops, ~2ns)
 *   + kmer_fwd update (~0.5ns) + tok_read (~0.5ns) + flat_cnt write (~0.3ns) = ~3.3ns
 *   → Just exceeds 3.33ns budget (300MHz) → Vivado AUTO-FREQ: 300→225.7MHz
 *   → KA = 76M × 1 / 225.7MHz = 0.337s (theory), actual 0.516s (effective II≈1.54)
 *   → No improvement over II=2@300MHz (0.516s≈0.517s)
 *
 * Fix:
 *   4-XOR hash (2.0ns critical path) → 2-XOR hash (1.0ns critical path)
 *   New EXTRACT_LOOP critical path: tok_read(0.5) + kmer_fwd(0.5) + 2XOR(1.0) + flat_cnt(0.3) = 2.3ns
 *   Margin vs 3.33ns budget: +1.03ns → II=1 at 300MHz should close timing
 *
 * Expected results:
 *   KA = 76M_tokens/inst × 1 / 300MHz = 0.253s (vs 0.517s II=2, vs 0.516s exp39)
 *   e2e = 0.037 + 0.253 + 0.138 + sort ≈ 0.037+0.253+0.138+0.385 = 0.813s (with 11-bit sort)
 *
 * Collision analysis for 2-XOR hash:
 *   k-mer 42-bit space, 9M unique k-mers: expected collisions ≈ 9M²/(2×4T) ≈ 10
 *   Negligible → unique count still accurate. CU routing: same cu for same k-mer ✓
 */

static inline uint64_t hash64_fe(uint64_t key, uint64_t mask)
{
#pragma HLS INLINE
    key ^= key >> 33;
    key ^= key << 21;
...
/* Stage 1: scan_fastq — unchanged from exp 37/38/39 */
...
/* Stage 2: unpack_tokens — II=1, same as exp 39 */
...
/* Stage 3: extract_kmers_8port — II=1 with 2-XOR hash (critical path ~2.3ns << 3.33ns) */
...
#pragma HLS DEPENDENCE variable=flat_cnt inter false
        ...
        if (kmer_len >= FE_K) {
            ...
            uint64_t h  = hash64_fe(y, km_mask);
            int      cu = (int)(h & (uint64_t)(FE_N_CU - 1));
            uint64_t c  = flat_cnt[cu];

            switch (cu) {
            case 0: km_base_cu0[c] = h; break;
            case 1: km_base_cu1[c] = h; break;
            case 2: km_base_cu2[c] = h; break;
            case 3: km_base_cu3[c] = h; break;
            case 4: km_base_cu4[c] = h; break;
            case 5: km_base_cu5[c] = h; break;
            case 6: km_base_cu6[c] = h; break;
            case 7: km_base_cu7[c] = h; break;
            }
            flat_cnt[cu] = c + 1;
        }
    }
...
/* Top-level kernel — same interface as exp 37/38/39 */
